...
void singleGrover(unsigned regSize, unsigned index) {
	if (index > regSize)
		throw "Invalid target index.";
	if (!(regSize && !(regSize & (regSize - 1))))
		throw "Invalid register size.\nMust be power of 2.";
	if (regSize > (unsigned)pow(2, MAX_QbitS))
		throw "Invalid register size.\nQbits limit exceeded.";

	srand((unsigned)time(NULL));
	index = (index == 0) ? rand() % regSize + 1 : index;

	Qbit reg((unsigned)log2((double)regSize));
	...
	std::cout << "Qbit register:\n" << reg;
	std::cout << "Probabilities:\n";
	reg.printProbabilities();
	std::cout << "\nTarget index: " << index << "\n";
}

void multipleGrover(unsigned regSize, std::set<unsigned> indices) {
